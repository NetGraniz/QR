...
function mergeQrSettings(value: unknown): QRSettings {
  ...
  return {
    ...
    frame: {
      ...
      ...(isRecord(value.frame) ? value.frame : {}),
      color: isRecord(value.frame) && isHexColor(value.frame.color) ? value.frame.color : DEFAULT_QR_SETTINGS.frame.color,
      thickness: isRecord(value.frame) ? clampNumber(value.frame.thickness, 0, 24, DEFAULT_QR_SETTINGS.frame.thickness) : DEFAULT_QR_SETTINGS.frame.thickness,
      radius: isRecord(value.frame) ? clampNumber(value.frame.radius, 0, 80, DEFAULT_QR_SETTINGS.frame.radius) : DEFAULT_QR_SETTINGS.frame.radius,
      padding: isRecord(value.frame) ? clampNumber(value.frame.padding, 0, 80, DEFAULT_QR_SETTINGS.frame.padding) : DEFAULT_QR_SETTINGS.frame.padding,
    },
    caption: {
      ...DEFAULT_QR_SETTINGS.caption,
      ...(isRecord(value.caption) ? value.caption : {}),
      color: isRecord(value.caption) && isHexColor(value.caption.color) ? value.caption.color : DEFAULT_QR_SETTINGS.caption.color,
      size: isRecord(value.caption) ? clampNumber(value.caption.size, 10, 48, DEFAULT_QR_SETTINGS.caption.size) : DEFAULT_QR_SETTINGS.caption.size,
    },
